class Solution:
    def maxSubArray_two(self, nums):

        if len(nums) == 1:
            return nums[0]

        m_r = self.maxSubArray_two(nums[1:])
        return max(m_r, nums[0], (m_r + nums[0]))
    ...
    def maxSubArray(self, nums):
        ...
        l = nums[:]
        r = [((i, i), l[i]) for i in range(len(l))]
